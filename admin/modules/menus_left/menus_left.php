...
if($vAct=='del' && !empty($vID)) {
	$vSQL_Del = "DELETE FROM menus_left WHERE ID='$vID'";
...
if(isset($_POST["btnSubmit"])) 
{
	if($vAct=='add' || $vAct=='edit') 
	{
		...
		if($vAct=='add') {
			$vSQL_Ins = "INSERT INTO menus_left(ParentID_Left, ModuleName_Left, MenuName_Left, Path_Left, Visible_Left, Possition_Left)
			...
			if($vResult_Ins) {
				...
				$vSQL_Role_Sel = "SELECT MAX(ID) as MenuID FROM menus_left";
		...
		{
			$vSQL_Upd = "UPDATE menus_left 
				SET ParentID_Left = $vParentID,
					ModuleName_Left = '$vModuleName', 
					MenuName_Left = '$vMenuName', 
					Path_Left = '$vPath', 
					Visible_Left = $vVisible, 
					Possition_Left = $vPossition 
				WHERE ID=$vID
			";	
...
$eVisible = 0;
if($vAct=='edit') { 
	$vSQL_Edit = "SELECT * FROM menus_left WHERE ID=$vID";
	$vResult_Edit = mysql_query($vSQL_Edit);
	$vRow_Edit = mysql_fetch_array($vResult_Edit);
	
	$eParentID	= $vRow_Edit["ParentID_Left"];
	$eModuleName= $vRow_Edit["ModuleName_Left"];
	$eMenuName	= $vRow_Edit["MenuName_Left"];
	$ePath		= $vRow_Edit["Path_Left"];
	$eVisible	= (isset($vRow_Edit["Visible_Left"]) || !empty($vRow_Edit["Visible_Left"])) ? 0 : 1;
	$ePossition	= $vRow_Edit["Possition_Left"];
...
$vSQL_Sel = "SELECT * FROM menus_left ORDER BY Possition_Left ASC";
	...
	<tr>
		...
		<td>
			<select name="cboMenuParent">
				...
				<?php echo combobox_menu_Left($eParentID); ?>
			</select>
		</td>
	</tr>
	<tr>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table-result">
  ...
  <tr>
	...
	<td><?php echo $vRowSel["ModuleName_Left"]; ?></td>
	<td><?php echo $vRowSel["MenuName_Left"]; ?></td>
    <td><?php echo $vRowSel["Path_Left"]; ?></td>
    <td align="center"><?php echo $vRowSel["Visible_Left"]; ?></td>
	<td align="center"><?php echo $vRowSel["Possition_Left"]; ?></td>
